...
- `interactions/sandbox.ts` — the Sandbox flow: a scratch record → transcribe → Mistral round-trip (`sendToMistral`, a stub with a placeholder query) meant as a starting point for new interactions, with the shared building blocks documented inline.
...
`main.ts` boots the app in a sequence of setup calls, then relies on the statically-declared panel entity:
...
5. `setupSandbox()` (`src/interactions/sandbox.ts`) — registers the Sandbox flow's `left-trigger-listener`.
6. `setupPanel()` (`src/ui/uikit-panel.ts`) — registers the `uikit-panel` A-Frame component.
...
The `uikit-panel` component builds a themed, scrollable panel: title, a scroll container holding the body `Text`, and a row of action buttons. Buttons are declared by `BUTTON_CONFIGS` (`{ id, label }`) — one per flow: **`ask`** ("Record", vision Q&A), **`detect`** ("Detect", object detection), **`create`** ("Create", object generation), **`place`** ("Place", create + place by a real object), and **`sandbox`** ("Sandbox", the scratch flow) — and addressed by id. Module-level exports drive it without threading refs:
...
There are five flows. **Ask**, **Create**, **Place**, and **Sandbox** are voice-driven and share one **record → transcribe** path (`src/voice/recorder.ts`; the whole app shares one recorder via `getVoiceRecorder()`, so its single-flight guard serialises every voice flow), diverging only in what the transcript feeds into; **Detect** is not voice-driven (a single button press, no transcript). The Mistral API calls live in `src/api/mistral.ts` (`transcribe`, `chat`, `streamChat`, model-id constants) and Google Vision in `src/api/vision.ts` (`localizeObjects`); `src/interactions/listeners.ts` wires Ask/Create, `src/interactions/detection.ts` wires Detect, `src/interactions/placement.ts` wires Place, and `src/interactions/sandbox.ts` wires Sandbox. The `environment` camera capture is shared via `src/camera.ts`.
...
- `left-trigger-listener` (`src/interactions/sandbox.ts`) — left controller trigger (`triggerdown`/`triggerup`) is hold-to-talk for the **Sandbox** flow. Sits on the left `meta-touch-controls` entity alongside the other left-hand listeners. The trigger is also the uikit panel's click (`src/ui/pointer.ts`), so both fire.
